...
exports.iterate = exports.flatten = exports.stringify = exports.init = exports.tail = exports.at = exports.max = exports.min = exports.equals = exports.isUnbounded = exports.length = exports.has = exports.intersect = exports.subtract = exports.append = exports.initialize = exports.normalize = exports.parse = void 0;
/**
...
const parse = (data, options) => {
    ...
    return (0, exports.normalize)(unsorted);
};
exports.parse = parse;
/**
...
const normalize = (data) => {
    ...
    if (typeof data === 'number')
        return (0, exports.normalize)([data]);
    for (const r of data) {
        let newRange;
        if (typeof r === 'number') {
            newRange = [r, r];
...
const initialize = (data, options) => {
    return typeof data === 'string' ? (0, exports.parse)(data, options) : (0, exports.normalize)(data);
};
...
const findOverlap = (data, target) => {
    ...
    const countOverlap = (lo) => {
        let count = 0, tmp, union = target;
        while (lo + count < data.length &&
    ...
    if (data.length > 0 && t0 < data[0][0] - 1) {
        ...
        let imin = 0, imax = data.length - 1;
        while (imax >= imin) {
            const imid = imin + Math.floor((imax - imin) / 2);
            if ((imid === 0 || t0 > data[imid - 1][1] + 1) &&
                t0 <= data[imid][1] + 1) {
...
const at = (data, index) => {
    ...
    for (let j = start; j >= 0 && j < data.length; j += delta) {
        ...
        const len = r[1] - r[0] + 1;
        if (i + len > nth)
            return delta > 0 ? r[0] + nth - i : r[1] - nth + i;
        i += len;
    }
    return undefined;
};
 ...
 */
const tail = (data) => {
    const m = (0, exports.min)(data);
    if (m === -Infinity)
        throw new RangeError('tail() was invoked on an unbounded MultiRange which contains -Infinity');
    if (m === undefined)
        return data;
    return (0, exports.subtract)(data, [[m, m]]);
};
exports.tail = tail;
/**
 ...
 */
const init = (data) => {
    const m = (0, exports.max)(data);
    if (m === Infinity)
        throw new RangeError('init() was invoked on an unbounded MultiRange which contains Infinity');
    if (m === undefined)
        return data;
    return (0, exports.subtract)(data, [[m, m]]);
};
 ...
 */
const stringify = (data, options = {}) => {
    const { individualThreshold = 1 } = options;
    const wrap = (i) => (i >= 0 ? String(i) : `(${i})`);
    const ranges = [];
    for (let r of data) {
        if (r[0] === -Infinity) {
            if (r[1] === Infinity) {
...
const flatten = (data) => {
    ...
    const result = new Array((0, exports.length)(data));
    let idx = 0;
    for (let r of data) {
 ...
 * [...iterate([[-1, 2]])]; // [-1, 0, 1, 2]
 */
const iterate = (data, options = {}) => {
    const { descending = false } = options;
    if ((0, exports.isUnbounded)(data))
        throw new RangeError('Unbounded ranges cannot be iterated over');
    return {
        [Symbol.iterator]: () => {
            let i = descending ? data.length - 1 : 0, curRange = data[i], j = curRange ? (descending ? curRange[1] : curRange[0]) : undefined;
            return {
                next: () => {
